...
static void
save_customlist_data (const char *file,
		      const char *key,
		      const char *id)
{
	GKeyFile *cfg;
	...
	cfg = g_key_file_new ();
	g_key_file_load_from_file (cfg, file, 0, NULL);

	tok = g_strsplit (key, "/", 2);
	g_key_file_set_string (cfg, tok[0], tok[1], ve_sure_string (id));
	g_strfreev (tok);

	config_save (cfg, file, NULL);
...
combo_selected (GtkComboBox *combo, GreeterItemInfo *item)
{
	...
	if (strcmp (item->id, "session") == 0) {
		...
		save_customlist_data (file, item->id, id);
	}
}
