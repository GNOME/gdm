		   ...
		   const char *x11_display)
{
	...
	g_debug ("Unlocking ConsoleKit session for %s on %s", user, x11_display);
		 ...
		 char         **cookiep)
{
	...
	g_debug ("Opening ConsoleKit session for %s", pwent->pw_name);
	...
	add_param_string (parameters, "host-name", hostname);
	...
	add_param_string (parameters, "session-type", session);
        ...
        add_param_string (parameters, "x11-display-device", display_device);
