...
// TODO: Not yet, actually
#define GDM_DYN_UID_MIN ((uid_t) 60578)
#define GDM_DYN_UID_MAX ((uid_t) 61090)
#define GDM_DYN_UID_COUNT ((size_t)(GDM_DYN_UID_MAX - GDM_DYN_UID_MIN) + 1)
          ...
          GError              **error)
{
        ...
        do {
                ...
                if (store->next_alloc < GDM_DYN_UID_MAX)
                        store->next_alloc += 1;
                else
                        store->next_alloc = GDM_DYN_UID_MIN;
               ...
               const char          *preferred)
{
        ...
        for (size_t i = 2; i <= GDM_DYN_UID_COUNT; i++) {
        ...
        /* Reaching this should be impossible, because it means that we
         * allocated more than DYN_UID_COUNT users. */
                 ...
                 GError     **error)
{
        ...
        /* Since the home directory is shared among multiple users, we use the
         * group that they all share as the owner of the home dir. We've also
         * adjusted the umask for our dynamic users appropriately */
        if (chown (home, 65534 /* nobody */, gid) < 0) {
                int errsv = errno;
                g_set_error (error,
                             G_IO_ERROR,
                             g_io_error_from_errno (errsv),
                               ...
                               GError              **error)
{
        ...
        if (!ensure_home_dir (home, grp->gr_gid, error))
                return FALSE;

        user = dynamic_user_new (username, display_name, home, uid, grp);

        g_mutex_lock (&store->mutex);
        g_hash_table_insert (store->by_name, g_steal_pointer (&username), user);
        g_hash_table_insert (store->by_uid, &user->uid, user);
        g_mutex_unlock (&store->mutex);

        /* Now that we're advertising the user via userdb, we can safely
         * unlock the user database. Anything trying to allocate a UID will
         * see that our UID is in use and pick something else. */
        ulckpwdf ();

        *ret_username = user->username;
        *ret_uid = user->uid;
        return TRUE;
}
...
gdm_dynamic_user_store_init (GdmDynamicUserStore *store)
{
        ...
        store->by_name = g_hash_table_new_full (g_str_hash, g_str_equal, g_free,
...
gdm_dynamic_user_store_dispose (GObject *object)
{
        ...
        if (store->worker != NULL) {
                ...
                g_thread_join (g_steal_pointer (&store->worker));
        }
                           ...
                           sd_json_variant **ret)
{
        return sd_json_buildo (ret, SD_JSON_BUILD_PAIR_OBJECT ("record",
                ...
                SD_JSON_BUILD_PAIR_UNSIGNED ("umask", 0202),
                    ...
                    void                      *userdata)
{
        ...
        store = GDM_DYNAMIC_USER_STORE (g_weak_ref_get (&ctx->store));
        if (store == NULL)
                return -ECANCELED;
        ...
        if (params.uid != 0)
                ...
                while (g_hash_table_iter_next (&iter, NULL, &value)) {
                        ...
                        if (found != NULL) {
                                ...
                                r = sd_varlink_notify (call, ret);
                                if (r < 0)
                                        return r;

                                ret = sd_json_variant_unref (ret);
                        }
                    ...
                    void                      *userdata)
{
        ...
        if (params.username) {
                ...
                if (test_group_membership (user, params.groupname))
                        return reply_user_group (call, FALSE, user);
        } else {
                GHashTableIter iter;
                gpointer value;
                ...
                while (g_hash_table_iter_next (&iter, NULL, &value)) {
                        ...
                        if (test_group_membership (next, params.groupname)) {
                                if (user != NULL) {
                                        r = reply_user_group (call, TRUE, user);
                                        if (r < 0)
                                                return r;
                                }
...
gdm_dynamic_user_varlink_worker (gpointer data)
{
        ...
        /* The default behavior is what we want: it quits the event loop */
        ...
        r = sd_varlink_server_listen_address (server,
                                              "/run/systemd/userdb/" GDM_SERVICE_ID,
